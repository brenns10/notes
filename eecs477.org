...
As discussed on Friday, the final exam will cover topics from the second half of
the class.  As prep work, I will put here a general listing of topics from the
second half, as well as internal links to notes about them.

Topics (most recent first):

- Paging: [[paging1][First Day]], [[paging2][Second Day]]
  - [[marking-algorithm][Marking Algorithm]]
- Amortized Analysis: [[aa1][First Day]] (mostly introduction), [[aa2][Second Day]]
  - [[aggregate-analysis][Aggregate Analysis]]
  - [[accounting-method][Accounting Method]]
  - [[potential-method][Potential Function Method]]
- Max Cut: [[maxcut][Lecture]].  The second day was Wednesday before Thanksgiving; was not
  present.
- Maximum Satisfiability: [[maxsat1][First Day]], [[maxsat2][Second Day]]
  - [[random-assignment][Random Assignment Algorithm]]
  - [[maxsat-randomized][LP-Based Randomized Algorithm]]
- Hitting Set: [[hittingset1][First Day]] (definition, 2 algorithms), [[hittingset2][Second Day]] (repeated
  execution and analysis of combined algorithm).
  - [[hittingset-primaldual][Primal Dual Algorithm]]
  - [[hittingset-random][Randomized Algorithm (v0.1)]]
  - [[hittingset-repeatedrandom][Repeated Randomized Algorithm]]
  - [[hittingset-summary][Summary of Algorithms]]
- Game Tree Evaluation: [[gte1][First Day]], [[gte2][Second Day]], [[gte3][Third Day]]
- Two Person Zero Sum Games: [[tpzsg1][First Day]], [[tpzsg3][Second Day]], [[tpzsg4][Third Day]]
  - [[tpzsg-def][Definition]]
  - [[tpzsg-saddle][Saddle Points]]
  - [[tpzsg-mixedsaddle][Mixed Strategy Saddle Points]]
  - [[tpzsg-lp][Linear Program for Maximum Worst-Case Payoff]] ([[tpzsg4][and again]])
  - [[tpzsg-vonneuman][Von Neuman Theorem]]
  - [[yao][Yao's Principle]]
- Game Theory Intro: [[gt1][First Day]], [[gt2][Second Day]], [[gt3][Third Day]]
  - [[gt-mixed][Mixed Strategies]] (and the penny game)
  - [[gt2][Strategic Form]]
  - [[gt-ext][Behavioral/Extensive Form]]
- MCNF (some of this was before midterm, but oh well):
  - [[mcnf1][First Day]] (definition, high level stuff)
  - [[mcnf2][Second Day]] (transformations, residuals, LP, unimodularity)
  - [[mcnf3][Third Day]] (duality)
  - [[mcnf4][Fourth Day]] (duality again)
  - [[mcnf5][Fifth Day]] (applications to shortest paths, dynamic programming, circulation)
  - (all MCNF above this point was "pre-midterm", now for "post-midterm",
    i.e. the algorithms)
  - [[mcnf6][Sixth Day]] (assumptions of algorithms, SSP)
  - [[mcnf7][Seventh Day]] (Runtime of SSP, Capacity Scaling Algorithm)
  - [[mcnf8][Eighth Day]] (CS algorithm, plus runtime)
- [[maxflowmincut][Max Flow == Min Cut]]

[[dual-shortcut][Shortcut table for taking duals.]]
...
** Paging Continued
   ...
   - $n$ pages in process, main memory holds \(k<n\) pages.
   - Process makes requests for pages, if not in main memory, we need to read
     from disk and maybe evict a page from main memory.
   - Need to come up with an strategy for evicting pages that will minimize page
     faults.
   - If we knew all the page requests ahead of time, there would be a relatively
     simple algorithm that could be applied to minimize page faults.
   - Problem: we don't know page requests ahead of time.  So we can think of
     this as an adversarial game.
   - When \(k<n\) (the normal case), the process can force the OS to page fault
     on every single request by simply requesting the page that was just
     evicted.
   - So, it seems like there's not much we can do with a deterministic algorithm
     for the general case.

   We would like to come up with an approximation algorithm and do an amortized
   analysis.
   - $c(G)$: number of page faults of algorithm $G$ on sequence $\sigma$.
   - OPT: optimum page replacement strategy on $\sigma$.
   - \(c(G) \le \alpha + c(OPT) + \beta \:\:\: \forall \sigma \)
   - Need an online algorithm (i.e. it has no future knowledge).  OPT has the
     advantage of future knowledge.

*** Marking Algorithm
    <<marking-algorithm>>
    - Associates a bit with every page in main memory.  Initially, all $k$ slots
      are unmarked.
    - When page $p$ is requested:
      - if $p \not\in$ main memory:
        - if all pages are marked, unmark all
    ...
    1. Initial main memory (none are marked):
       | 1 | 2 | 3 |

    2. Request page 4.  2 is arbitrarily chosen to be evicted
       | 1 | 4* | 3 |

    3. Request page 2.  1 is arbitrarily chosen to be evicted
       | 2* | 4* | 3 |

    4. Request page 4.  Nothing changes
       | 2* | 4* | 3 |

    5. Request page 1.  3 is the only page that can be evicted.
       | 2* | 4* | 1* |

    6. Request page 3.  All pages must be unmarked, and then 4 is arbitrarily
       chosen to be evicted.
       | 2 | 3* | 1 |

    Ok.  Looks pretty darn simple.  This marking algorithm tries to emulate LRU
    (Least Recently Used), but with only one bit of memory to store how recently
    used.  Or you could think of it the other way, that M is a generalization of
    LRU, because M has a choice to evict "any" page, and LRU specifies that the
    page it evicts is the least recently used.

    For the purpose of analysis, we will define a "Phase".  P_0 is at the start.
    P_i is a maximal subsequence of requests containing $k$ distinct pages.

    *Theorem:* \(c(M) \le k c(OPT) + k\)

    *Proof:* In each phase, $M$ incurs no mare than $k$ page faults (once a page
    is marked, it stays in main memory for the rest of the phase).

    For the purpose of analyzing the optimal strategy, we will call the first
    requested page of phase $q$.  We will define $Q$ to be the remaining page
    requests of the phase, plus the next one.  $Q$ contains exactly $k$ distinct
    pages that are not equal to $q$.  By the end of $Q$, OPT will have to incur
    a page fault (because $q$ is in main memory, and it's impossible to have all
    $k$ of the other distinct pages in $Q$).

    The sequence of page requests will contain some number $m$ of complete
    phases, followed maybe by an additional $\sigma$, or maybe not.  So there are $m$
    complete phases for the marking algorithm, and in the worst case there are
    $m-1$ regions $Q$.  In each phase, the M algorithm incurs cost \(\le k\) while
    the optimal incurs cost \(\le 1\).  In the final phase, the M algorithm incurs
    cost \(\le k\), while the optimal may or may not incur any cost.

    So, we can easily see that \(c(M) \le k c(OPT) + k\)

    Paging has tons of interesting "confluences" of concepts from this class:
    ...
    Sadly, we didn't get a chance to cover it all.

** Final

   He will be asking questions about the second part of the course (after the
   midterm).  You will be pretty screwed if you don't know LP anymore, but there
   won't be any explicit questions on the content of the first half.  The exam
   will be open book and note (!!).  Question types:

   - Explain (insert some topic/algorithm/proof).
     - Open book, so he may ask questions about the proof to show that you do
       understand the proof, and aren't just reading the proof for the first
       time.
   - Simple problems - as in do thing x.
   - Homework problems may be asked verbatim!  This could be a "personalized"
     strategy, so you should look at what you did wrong in the homework and make
     sure you understand what you did wrong.

   The rules about what you can bring are mostly guidelines, cause you'll be
   solving problems in front of (and with) him.
...
*** Aggregate Analysis
    <<aggregate-analysis>>
   We were doing amortized analysis of the binary counter.  Over all 2^n
   increments, you'll get the following costs:

   - Cost 1 for 2^{n-1} increments
   - Cost 2 for 2^{n-2} increments
   - Cost 3 for 2^{n-3} increments
   - \dots
   - Cost n for 2^{n-n} (=1) increments

   So the average cost is:

   \begin{align*}
     \text{avg. cost} = \frac{1}{2^n} \sum_{i=1}^n 2^{n-i} i
   \end{align*}

   Actually, the cost n action happens twice.  Let's do this in terms of bits:

   - Bit 1 examined 2^n times
   - Bit 2 examined 2^{n-1} times
   - \dots
   - Bit n examined 1 (+1) times

   So the total and average costs are:

   \begin{align*}
     \text{total cost} &= 1 + \sum_{i=0}^n 2^i = 2^{n+1} - 1 + 1 = 2^n+1 \\
     \text{average cost} &= \frac{2^{n+1}}{2^n} = 2
   \end{align*}

*** Accounting Method
    <<accounting-method>>

    We associate costs with actions on the data structure.

    - When you increment the counter from 0, you travel to the first bit, and
      flip it.  You leave a "candy".  Your cost is 1 bit traveled plus 1 candy
      placed = 2.
    - When you increment from 1 to 2, you travel to the first bit and flip it to
      zero, and eat that candy.  Then, you travel to the second bit, flip it to
      one, and leave a candy there.  Your cost is 2 travel, 1 candy placed, and
      1 candy eaten, for a total of 2.
    - When you increment from 2 to 3, you travel to the first bit, flip it to
      one, and leave a candy.  Your cost is 1 travel, 1 candy, for a total of 2.
    - When you increment from 3 to 4, you travel to the first bit, flip it to 0,
      eat the candy.  Same thing at the second.  Finally, at the third, you flip
      it to one, and leave a candy.  Your cost is 3 travel, 1 candy placed, and
      2 candy eaten, for a total of 2.

    This pattern continues: no matter whan increment you do, you will always
    have a cost of (at most) 2, which happens to be the same as we got above.

    How does this work?  The idea is that when you flip a bit to 1, you also
    account for the fact that later on, you will be flipping it to zero (this is
    the act of leaving a candy).  Then, when you actually do that operation
    later on, you "eat the candy", or in other words you don't have to pay that
    cost.  This is actually really cool.

*** Potential Function Method
    <<potential-method>>

    - Data structure D_i: state after $i$th operation.
    - \Phi: maps state of data structure \to real numbers.  This is a potential
      function.
    - c_i: cost of $i$th operation

    The total cost of all operations on the data structure (if there are $m$
    operations) is simply:

    \begin{align*}
      \text{total cost} = \frac{1}{m} \sum_{i=1}^m c_i
    \end{align*}

    Now, we define \(\hat{c}_i\), to be the "amortized" cost of the \(i\)th
    ...
    \begin{align*}
      \sum_{i=1}^m \hat{c}_i &= \sum_{i=1}^m (c_i - \Phi(D_{i-1}) + \Phi(D_i)) \\
                    &= \sum_{i=1}^m c_i - \Phi(D_0) + \Phi(D_m) \\
                    &\ge \sum_{i=1}^m c_i
    \end{align*}

    The last inequality holds whenever \(\Phi(D_m) \ge \Phi(D_0)\).  By convention, most
    papers set \(\Phi(D_0) = 0\), and ensure that \(\Phi(D_m) \ge 0\) at all times.

    With this formulation, we can come up with any potential function we like
    (that satisfies the above condition), and then sum up the amortized costs
    instead of the actual costs, and get a bound on the total cost.

    In the binary counter, we let the potential function be the number of 1's in
    the counter.  We can see that the value of \(\hat{c}_i\) is always 2.

    In this case, the potentials are pretty much the same as the candies in the
    Accounting Method.  This is not always the case; you can view the potential
    method as more general than the accounting method, because it can be used in
    situations where there's no clear way to "assign candies" in strategic
    locations like we did earlier.

** Paging Problem

   <<paging1>>

   This problem has to do with memory management in operating systems.
   - We have space in main memory for $k$ pages.
   - We have a process that has a working set of $n$ pages.
   - The \(n-k\) pages that are not stored in main memory are stored safely on
     disk.

   We're going to view the process as an adversary that requests pages out of
   its working set.  If the OS has it in main memory, nothing needs to happen.
   Otherwise, it should be loaded from the disk into main memory.  The other
   problem is that the OS will (probably) have to evict a page from main memory
   to make room for the page.

   The OS would like to maximize the number of hits, or minimize the number of
   page faults (where it loads the page from disk).

   Our worst case analysis would be simple - in the worst case, there is a page
   fault on every request, so we have a cost of 1 disk access for every page
   request.

   We can see that a randomized algorithm could prevent the worst case for a
   process.  But maybe we'd like to do an amortized analysis for a deterministic
   algorithm.  Next time we'll talk about that.

* 2015-11-30 Monday

** Homework 5 recap

   Here is a nicer overview of the steps from the capacity scaling algorithm,
   for when you're actually stepping through it (instead of the algorithm as
   presented earlier, which is a bit difficult to interpret at times).

   For each delta scaling phase:
   - Copy over the residual network (nodes and arcs).
   - Copy over the node potentials (\pi) from the previous iteration, as well as
     the reduced costs, upper bounds, and excesses.
   - Any new edges (not in the previous residual) should be drawn, and have
     their reduced costs computed.  Chances are, some of these will have
     negative reduced costs.
     - You do arc reversal on those arcs, which will change the excesses.  This
       is a good time to redraw the residual.  (at least, this is when
       Dr. Liberatore redraws it).
   - Select an excess and deficit vertex to do an augmentation on.  You will
     then recompute the node potentials (\pi) and the reduced costs (c_{ij}^\pi).
     Also make sure to add in the reversed edges.
     - Repeat this until all your excesses and deficits are less than \Delta.

   It seems like the other thing I missed was that you should assume that there
   is a path from 1 to 8, of very large cost.  In his example (which he started
   last class), it looks like he made the cost 15.  I'll have to get some input
   from Jeff on how he did that in class last Wednesday.

** Amortized Analysis

   <<aa1>>

   Binary counter: suppose you have an $n$ bit counter.  Initially, it is set to
   all 0's.  We will pretend this counter is a data structure that supports only
   one operation, which is increment.  This does your typical increment
   operation (flip the bit and carry if it was 1).  An increment operation has a
   cost defined by the number of bits you flip.

   The worst case cost for an operation is going to be $n$.  However, it looks
   like most of your costs are going to be much smaller.  Half of them will have
   cost 1.  So amortized analysis will be a way of doing worst-case analysis
   that involves finding the cost over an entire sequence of operations.
...
** Max Cut
   ...
   - Graph \(G=(V, E)\)
   - *Def:* A cut is a partition of \(V\) into \(S\) and \(V-S\).
   - *Def:* An edge \(e = \{u,v\}\)\) crosses cut \(S, V-S\) iff \(u \in S\), \(v
     \not\in S\).
   - Let \(a_{ij}\) be the weight (\ge 0) associated with edge \(\{i,j\}\).
   - The problem is to find a cut that maximizes \(\sum_{i,j\text{ crossing cut}} a_{ij}\)

   This problem is NP-hard, so we'd like to find an approximation algorithm.
   Note a few things.  First, the max cut for a bipartite graph is just the cut
   that separates the two sets of edges.  Also, in general, we can assume that
   we are working with the "complete" graph, and that the edges are 0 if there
   was no edge in the original graph.

   First Claim: there is a randomized algorithm with an approximation ratio of
   \(\frac{1}{2}\).

   This algorithm simply assigns each node to $S$ with probability
   \(\frac{1}{2}\).  The probability that an edge crosses the cut is
   \(\frac{1}{2}\).  So the expected probability is \(\frac{1}{2} \sum a_{ij}\), which
   is greater than or equal to half the maximum possible optimal (\(\sum a_{ij}\)).

   Now, let's look at the integer linear program.  First, our variables:
   ...
   \begin{align*}
     \max \sum_{i<j} a_{ij} \frac{1} - x_i x_j}{2}  \text{ s.t. } x_i \in \{\pm1\} \forall i \in V
   \end{align*}

   Note that \(x_i x_j\) is positive 1 if the edge doesn't cross, and negative 1
   if it does.  So \(1 - x_i x_j\) is 2 if the edge crosses, and 0 otherwise.  So
   then we divide by the above to get the "indicator" that the edge crosses the
   cut.

   This program is a bit of a downer because it's quadratic, *and* linear.
   We're gonna stick with it, We're going to relax the integer condition on x,
   in a very weird way.

   Say that instead of $x$ being scalar, we used $\vec{v}$, a vector on the unit
   sphere in \(R^n\).  If they are opposite each other, then the dot product will
   be -1, and if they are the same, the dot product will be 1.  And we'll also
   relax the "integer" constraint on the $\vec{v_i}$.

   Okay.  Let's now say we have \(B = (v_1, v_2, \dots, v_n\).  Let \(y = B^T B\):
   ...
   So now our objective is \(\max \sum_{i\lt j} a_{ij} \frac{1 - y_{ij}}{2}\).  I guess we need
   that the matrix is positive semidefinite.  So I guess we have a semidefinite
   program.  We can solve this SDP to within some $\epsilon$ in time polynomial in $n$
   and \(\log \epsilon\).  Since \epsilon is a constant here, we're good.  This will give us
   vectors \(v_i^*\).

   Our job will be to separate the vectors into two groups.  Turns out we'll
   just select a random hyperplane.  Woo randomness.
...
** MAX SAT
   ...
   - $p$ clauses, \(c_1, \dots, c_p\)
   - \(c_i = x_1 \lor \bar{x_2} \lor x_3\) (for example)

   Continuing on:
   - Let \(k(i)\) be the number of literals in clause $i$.
   - Let \(S_i^+ =\) the set of $x$ variables that are not negated.
     - For the example $c_i$ above, \(S_i^+ = \{x_1, x_3\}\)
   - Let \(S_i^- =\) the set of $x$ variables that are negated.
     - For the example $c_i$ above, \(S_i^+ = \{x_2\}\)

   In max sat, we want to maximize the number of clauses that are satisfied.
   Our initial randomized algorithm (which we applied when \(k(i) \ge k = 2\))
   assigned each variable a value with probability \(\frac{1}{2}\).  This gave a
   $\frac{3}{4}$ approximation algorithm, which is tough to beat.  But the
   performance is bad when $k=1$.

   So now we'll examine an algorithm based on a linear program.  Let's use the
   following notation:
   ...
   Now, here is the working ILP formulation:

   \begin{align*}
     \max \sum_{i=1}^p z_i & \text{ s.t.} \\
     \sum_{j \in S_i^+} y_j + \sum_{j \in S_i^-}(1-y_j) &\ge z_i \\
     0 \le z_i &\le 1 \\
     y_j &\in \{0, 1\} \\
   \end{align*}

   Our $z_i$ are variables such that:
   ...
   When we do the LP relaxation, $y_i$ can range from 0 to 1.  Then, our
   randomized algorithm will have \(Pr(x_j = T) = y_j^*\).  So, the probability
   that a clause is satisfied is:

   \begin{align*}
     Pr(C_i = T) &= 1 - \left(\prod_{j \in S_i^+} (1-y_j^*)\right) \left(\prod_{j \in S_i^-} y_j^*\right) \\
     &\ge \left(1 - \left(1 - \frac{1}{k(i)}\right)^{k(i)}\right)z_i^* \\
     &\ge \left(1 - \frac{1}{e}\right)z_i^*
   \end{align*}

   So, the expected number of clauses satisfied is \(\sum_{i=1}^p \left(1 -
   \frac{1}{e}\right)z_i^*\).  The best you can do is \(\sum_{i=1}^p z_i^*\), so the
   approximation ratio is \(\left(1 - \frac{1}{e}\right) \approx 0.63\).

   This isn't as good as the original algorithm, which had an approximation
   ratio of 0.75.  But it deals better with the cases of small $k$.  Of course,
   the downside is that we have to solve a linear program.  Which kinda stinks.
   So our best algorithm is just to run both and pick the best.

   OK, what can we say about the expected performance of this "best" algorithm?
   Well, the algorithm has a performance of \(\max\{n_1, n_2\}\), where the two
   $n$ variables are the performance of the two algorithms.  Expectations of
   max's are very hard, so we will just say that:

   \begin{align*}
     \max\{n_1, n_2\} &\ge \frac{n_1 + n_2}{2}\\
     &= \frac{1}{2} \left(\sum_{i=1}^p \left(1 - \frac{1}{2^{k(i)}}\right) \sum_{i=1}^p \left(1 - \left(1 - \frac{1}{k(i)}\right)^{k(i)}\right)z_i^*\right) \\
     &\ge \frac{1}{2} \sum_{i=1}^p z_i^* \left(1 - \frac{1}{2^{k(i)}}\right) \left(1 - \left(1 - \frac{1}{k(i)}\right)^{k(i)}\right)\\
     &\ge \frac{1}{2} \sum_{i=1}^p z_i^* \frac{3}{2} \\
     &= \frac{3}{4} \sum_{i=1}^p z_i^* \\
   \end{align*}

   That step up above where we replaced the big stuff with $\frac{3}{2}$ can
   actually be proven easily by induction on $k$.  But it's pretty darn cool
   result, because it's not true in general that the expectation of a max
   operation is just the max of the respective expectation.  It works out this
   way because the two algorithms make up for each other's weaknesses (k=1).
...
** "Math Facts"
   ...
   - in the limit as \(k\to\infty\), the right side is equality.
...
** Maximum Satisfiability
   ...
   - Have $c_i$, a bunch of clauses over $x_1, \dots, x_n$.
     ...
     - In general, a clause may only be a disjunction of variables (or their
       negations).
   - We would like to find a truth assignment to $x_1, x_2, \dots, x_n$ that maximizes
...
** Two Person Zero Sum Games (cont)
   ...
   \begin{align*}
     \sum_{i=1}^n x_i \sum_{j=1}^m H(i,j) y_j^* &\le \sum_{i=1}^n x_i w^* \\
...
** Two person zero sum game
   ...
   This means that the first player's maximization is the second player's min.
   ...
   - Andrew's musings: A saddle point is a
     [https://en.wikipedia.org/wiki/Nash_equilibrium[Nash equilibrium]]. For
     pure strategies, there does not have to be a saddle point, but if you
     allow mixed strategies, then for every two-player, zero sum game, there
     is always a saddle point (from ECON 329).
...
** Game Theory!
   ...
   - Set \(S^i \) of pure strategies for the \(i^{\text{th}}\) player.
   - Payoff function \(H: S^1 \times S^2 \times \dots \times S^n \to R^n \)
