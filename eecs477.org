...
** Max Flow Reduction: Matrix Rounding
  ...
  Create a new matrix $B$ such that $b_{ij}$ is either $\floor{a_ij}$ or
  $\ceil{a_ij}$. However, $B$ should also obfuscate the sums of both the
  columns and rows of $A$ in the same manner (so, $\sum_{i}b_ij$ should be
  either $\floor{\sum_{i}a_ij}$ or $\ceil{\sum_{i}a_ij}$, and so on, for each
  row and column).
  ...
  column and row sums. Then, make an arc going from $a_{ij}$ to $c_j$, with
  lower bound of $\floor{a_{ij}}$ and $\ceil{a_{ij}}$. Also, make an arc going
  ...
  lower bound $\floor{c_j}$ and upper bound $\ceil{c_j}$. Similarly, make an
  arc going from a source node $s$ to each $r_i$. Each arc will have upper
  bound $\floor{r_i}$ and upper bound $\ceil{r_i}$.
...
** Reductions
  ...
  - We know the value of the max flow \(\le \sum u_{si}\) and the value of the y
    flow \(\le \sum y_{si} = \sum b_{i} \mathbb{1}_{bi \ge 0} \ge) vlaue of max flow. So,
