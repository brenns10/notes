...
** Embedded Methods
   ...
   - Alter the objective functions of learning algorithms to simultaneously
     select features and learn the classifier.
   - Best of both worlds, but requires altering classifier.

   How do we modify objectives?
   - Typically objective functions aim to minimize some loss function.
   - We need to add a criteria to encourage "sparseness" - that is, make more
     weights zero (zero weights mean you can just get rid of the feature).
   - Frequently you can modify the objective by adding a "zero-norm" term.  This
     is the number of weights that are non-zero (which we would like to
     minimize).
     - EG: \(L'(w, x, y) = L(w, x, y) + \lambda ||w||_0\)
   - Unfortunately, this term is not differentiable.  However, it is reminiscent
     of other overfitting controls (like weight decay).

*** Adapting Overfitting Controls

   Relationship between overfitting control (OC) and feature selection (FS):
   - These two concepts have very similar objectives!
   - A classifier that is robust to overfitting will probably use a smaller and
     more concise (or logical) set of features.
   - A classifier that uses fewer features will likely be more generalizable.

   What if we used overfitting control techniques for feature selection?
   - For example, what we had in the ANN: \(L'(w, x, y) = L(w, x, y) +
     \gamma ||w||^2\)
   - This is good, but it's possible to do better.
     - The square function "doesn't care" too much about relatively small
       values.
     - It penalizes the very high ones, but it doesn't encourage completely
       eliminating features.
   - The \(||w||^2\) term is called the $L_2$ norm.  Let's use a different
     penalty, the $L_1$ norm, (aka the Lasso).
   - The $L_1$ norm is simply the sum of the absolute values of the components of
     a vector.  It's not differentiable, but we can solve this using linear
     programming.
     - Instead of having unconstrained optimization using the L_1 norm in the
       objective, we can drop the L_1 norm into constraints.
     - The constraints would be of the form \(||w||_1 \le B\) (for L_1) and
       \(||w||_2 \le B\).  If you think of the geometry of these constraints,
       they're "cubes" and "spheres" respectively.
     - Imagine you're doing optimization with these two objectives.  With the L_1
       ...
       is zero.
     - On the other hand, the L_2 norm's "sphere" typically will find solutions
       where all weights are nonzero simultaneously.
     - There are some seriously good figures in the slides to explain this.
   - Tragically, the Lasso/L_1 is not differentiable (although it has some very
...
** Dimensionality Reduction
   ...
   Formal definition:
   - Given: a set of examples \((\vec{x}_i, y_i)\) over $D$ features.
   - Do: find a /subspace/ of dimension $d$, $d<D$ ... (see slides)

*** Principal Components Analysis

    - Given: a set of points \(\vec{x}_i\) centered in \(R^D\).
    - Find: an orthonormal set of $d$ ($<<D$) vectors \(w_j \in R^D\), and the
      ...
      W z_i\) minimize the /reconstruction error/:
    ...
    Solving PCA:
    - We can solve incrementally by asking for the first projection vector.
      ...
      covariance matrix of the examples.  The coefficients $z$ follow.
    - Then you can do this again for the "residuals".  Turns out this also has a
      closed form solution, which is eigenvector corresponding to the second
      largest eigenvalue.
    - This pattern continues.  Nifty.

    Problems with PCA:
    - PCA doesn't take labels into account.  So you might not find the "axis of
      variation" that accounts for the difference between the labels.
    - Can get around this with modifications to the process.

    How to pick the number of dimensions?
    - Plot the reconstruction error as a function of the number of dimensions.

    Other general PCA notes:
    - PCA needs standardized inputs.
    - Ignores class labels.
    - The components are typically not "sparse" in terms of the input features.
      So it's not easy to interpret the results or understand what features
      ought to be dropped.
    - Although there is a closed form solution, the decomposition is expensive
      (similar to random walk with restarts...), so there are iterative
      solutions that converge much faster.

* 2015-11-24 Tuesday
** Kalman Filtering
   - As discussed last time, we represent state as a vector of continuous
     variables.  The observations are modeled as a gaussian of a linear function
     of the states.  You also have a sensor distribution.
   - Filtering: estimate the state at time $t$.
   - The nice aspect of Kalman Filter is that it's linear Gaussian all the way
     down, so there are closed form solutions.
   - Parameter estimation can happen via iterative least squares.
   - There are many variations of Kalman Filters.

** Feature Selection
   - Classification algorithms need feature vectors.  If important features
     aren't present, they can't do anything.  Which means we usually take the
     "kitchen sink" approach - put in *all* the features!
   - However, including all the features may be bad.  Sampling artifacts may
     make irrelevant features /look/ relevant, and these will produce
     classifiers that are less generalizable.
   - Problem statement: Given a set of examples, find a subset $S$ of features
     (\(|S| \le D\)) so that, for any other subset \(S' \ne S\), the learned concept
     that uses $S$ /generalizes better/ than the learned concept that uses $S'$.
   - Problems with this problem:
     - Optimal solution is combinatorial.
     - chicken and egg - "to find relevant features, we need the target concept,
       but to find the target concept, we need relevant features."

*** Filter Methods

    - "Let's pretend these problems don't exist!  Maybe they'll go away."
    - Filter Methods don't involve the learning algorithm, so they avoid the
      chicken & egg problem.
    - Just apply a procedure before training, then train on the filtered
      features.
    - Typical procedure:
      - Score each feature according to some criterion.
      - Keep the $k$ highest scoring features.
    - Common criteria:
      - Mutual information / information gain
      - Probably correlation measures such as PCC
    - Issues:
      - Sometimes features aren't informative alone but they are together.
      - Sometimes features are "proxies" for each other, so they'll provide
        similar, duplicate information. (relevance, not redundancy).
    - Solutions: Switch to a sequential process, where you start by picking a
      feature and then add features w.r.t. their adjusted score (conditioned on
      the existing features).
    - Downsides:
      - More "comparisons"
      - Highly dependent on the initial feature choice -- can go "bad" quickly.
      - Computing conditional mutual information (\(I(X;Y|S)\) is difficult as
        the conditioned set of variables $S$ grows.
    ...
    - Pros:
      - Generally pretty efficient (due to their simplicity).
    - Cons:
      - May produce suboptimal results, due to heuristic scoring functions,
        calibration, sample sizes, etc.
      - Ignores the algorithm that will actually do classification

*** Wrapper Methods

    - These methods "search" the space of possible subsets, by repeatedly
      applying the algorithm and using the performance as a guide.
    - Can't search exhaustively, so heuristics, here we go!
    - Forward Feature Selection:
      - Start with empty set.
      - Add features as your search operators
      - Use classifier's generalization error as the cost function.
      - Termination condition is when the error doesn't improve.
    - Backward Feature Selection is the reverse - it starts with all the
      features and works backward
    - Note on implementation: feature selection needs to be done only on the
      training data, not on the whole set.  So, it is normal to do feature
      selection in each fold and get different feature sets.
    - We can use boosting as a wrapper to select features.  Typically this is
      ...
      - Note that at each iteration, you get a feature that is best at
        classifying the examples that were most misclassified with the
        previously selected features.
      - Each feature provides new and complementary information (like we tried
        to do with conditional mutual information before).

    Pros and Cons of Wrapper Methods:
    - Pros:
      - Provide very good performance because they are paired with the learner.
        They solve the chicken and egg problem iteratively.
      - Robust to missing data.
    - Cons:
      - Need lots of data to work well (cross validation)
      - Very computationally intensive!!
      - Need to use fast learners.
      - Boosting is a good middle ground.
...
** Hidden Markov Models
   ...
   Last time:
   - Hidden Markov Models
   - Forward algorithm for determining probability of an observed sequence.
   - Viterbi Algorithm for finding most likely sequence of states that could
     have emitted an observed sequence.

   Now:
   - Given some observations and a model structure, how to estimate parameters?

*** Learning Model Parameters

    - We have a set of observation sequences.  These sequences are i.i.d., but
      the elements of the sequences (obviously) are not.
    - Parameters are:
      - Emission distribution for each state: \(Pr[a | S = k]\).
      - Transition probability for each pair of states.
    - How do we do this?  Maximum Likelihood Estimation!

*** Simpler Problem

    - This is gonna be difficult due to the fact that we don't know the state
      sequences associated with the observed emissions.
      - /That's why it's a *Hidden* Markov Model!/
    - What if, instead we had "annotated" examples?  That is, each observation
      sequence comes with a list of states.
    - Then, we can simply count up the emissions for each state, and build a
      probability distribution out of it.
    - We can do something similar with the transitions.

*** Baum-Welch Algorithm

    - Sadly, the world is not a nice place, and it doesn't typically give us a
      state sequence.
    - This is /missing data/, and for generative probabilistic models, we can
      handle this with *expectation maximization*!
      - As a refresher, in the E step, we estimate values of missing values
      - Then in the M step, we do MLE
      - Then we iterate this
    - The algorithm here is Baum-Welch Algorithm.
...
**** M Step
     ...
     - Once we have the $E$ and $C$ values, the maximum likelihood estimation is
       simply then taking these expected counts and computing probabilities,
       just like we did when we had labeled sequences.

*** Handling Labeled Sequences

    - What if we have labeled sequences?  We've dealt with "emissions", but what
      if the sequence as a whole is labelled?
    - You just train a model for each class.  (Need to be careful to choose your
      samples for each class).
    - Then, to classify, you just get the probability out of each model, and
      choose the class with the higher probability.
    - Or, you could combine your structures into a single HMM, with a START
      ...
      and inspect the most likely path to see class label.

*** Discriminative HMM

    - Can do a similar discriminative model, if the generative HMM is overkill.
    - Called Conditional Random Fields.

*** Stochastic CFGs

    - What if the order of dependence cannot be bounded?
    - You may need a stochastic context free grammar.
      - you can think of HMM's as stochastic regular grammars.
    - Like regular/context free languages, HMMs and Stochastic CFGs have tons of
      similarities.  Algorithms exist to do similar things in both.
    - In fact, CYK algorithm can be used like the Viterbi algorithm to return
      the most likely "state" sequence, which is ridiculously cool, since we
      originally covered in in 343 as just a normal parsing algorithm.

** Discrete Time, Continuous State

   - Not all processes have discrete state.
   - Examples of processes with continuous state:
     - Robot that is tracking its location as it moves.
     - System that is tracking objects on radar.

*** Kalman Filtering

    - Specify process state using continuous variables.
    - To model the evolution of the process state, need a distribution
      \(Pr(s_{t+1} | s_t)\).
    - One possibility is to use a linear function of $s_t$ along with some
      Gaussian noise.

    Example:
    - Motion tracking: state has position and velocity.
    ...
    - Then, you model the probability distribution $X$ like this:
      ...
      \end{align*}
    - Above, we're modeling the next state given observations of the current
      state.
    - However, there's no guarantees on our sensors.
    - So, in general, we can also model the probability we observe $\vec{o}$
      given that we are in state $\vec{s}$.

*** Online Prediction

    With continuous state problems, we frequently would like to do some
    predictions "online":
    - Filtering: what is the current state given the observations so far?
      \(Pr(s(t) | o(1), \dots, o(t))\)
    - Prediction: what is the next state I am likely to be in? \(Pr(s(t+k)|o(1),
      \dots, o(t))\)
    - Smoothing: How likely was I to have been in some state in the past, given
      the observations so far? \(Pr(s(t-k)|o(1), \dots, o(t))\).
    - Most likely path: \(\arg \max_s Pr(s|o(1), \dots, o(t))\).
...
*** Filtering
    ...
    - The output of filtering at the previous state
...
** Sequential Supervised Learning
   ...
   - So far we've looked at data we could represent as feature vectors.
   - Now we'd like to go further and look at examples that are sequences of
     observations.
   - Examples:
     - NLP: Text classification, parsing, part-of-speach tagging
     - Protein family modeling
     - Speech recognition
     - Activity recognition

*** Sequential Data
    - Sequence of observations o_1, o_2, ..., o_n.
    - Each observation is drawn from a background alphabet/vocabulary
    - Given this data, we have many types of prediction problems:
      - Classify the whole sequence
      - Classify each element
      - Something in between...

*** Generative Process Model
    - We hypothesize that there is an underlying generating process we can
      model.
    - The process has a state that could be discrete or continuous.
    - The state evolves over time.
    - At discrete time points, we observe something about the state.
      - could be continuous time points, but less common

*** The -1st Approach
    - Just ignore that the data is sequential, and try to use a normal learning
      algorithm.
      - Sometimes this is a reasonable first choice.
    - This approach has met with some success, especially with Naive Bayes
      models of text classification.
      - Spam/ham, sport/finance
      - Use a "bag-of-words" approach - it doesn't take into account word order.

*** The 0th Approach

    - We'll accept that there is an underlying process, but not process the
      state.
    - We'll take into account the position of each feature/item in the model.
    - Position Specific Scoring Matrix
    - Need to have a constant length sequence.
    - Doesn't account for dependence on previous inputs

*** The kth Order Approach

    - Model dependence of order $k$, with a $k$ order hidden markov model.
    - This will take into account the $k$ previous examples in the "decision"
    - Essentially, a HMM is a probabilistic FSM that emits output
      probabilistically in each state.
    - It's a generative model!
    - It works by factoring the probability distribution of \(Pr[\{s_1, s_2, \dots,
      s_n\}, \{o_1, o_2, \dots, o_n\}]\).  This is a *time series*.  A first order HMM
    ...
    - Note that with higher order models, you condition on more than just the
      previous state in the "transition probability".
    - Aside:
      - What is "markov" about this?  Only considers the previous $k$ inputs.
      - What is "hidden"?  The states!
    - Inference:
      - What is the probability of a sequence?
      - What is the most likely sequence of states to produce an observations?
    - Learning:
      - How to estimate parameters? (given a model structure)
      - How to learn a structure?

**** Inference in HMMs

     - How to estimate \(Pr(\vec{o})\)?
     - Clearly, \(Pr(\vec{o}) = \sum_{\vec{s}} Pr(\vec{o},\vec{s})\) (which we already
       have above.
       - Problem: the number of potential state sequences that could generate
         $\vec{o}$ is pretty huge in general.  Exponential, actually.
     - Key observation that saves us: Many \(Pr(o, s)\)  functions share
       "prefixes", which we can store using dynamic programming.
     - Forward algorithm:
       - Let \(\alpha_k(i) = Pr(o_1, \dots, o_i, s_i=k)\)
         - This is the probability that the model has emitted the first $i$
           observations and is now in state $k$.
       - We would like to compute \(\alpha_{END}(n)\) (this assumes that we have a dummy
         end state for convenience).  This is the probability of the entire
         observation.
       - Construct a table of size $n$ by $m$, ($n$ is length of observed
         sequence, $m$ is number of states).
       - The forward algorithm is a dynamic programming procedure that will fill
         the table with \alpha values.
       - Initialize: \(\alpha_{START}(0) = 1\), and \(\alpha_k(0) = 0\) for \(k \ne START\).
       - Recursion: \(\alpha_k(i) = Pr(o_i | s_i = k) \sum_p \alpha_p(i-1) Pr(s_i=k | s_{i-1} = p)\)
...
**** Most Likely Path
     ...
     - The dumb way to solve it is to enumerate all possible state sequences.
     - The smart way is to use dynamic programming again, using an algorithm
       called the Viterbi algorithm:

     *Viterbi Algorithm*

     - Let \(\gamma_k(i) = Pr(o_1, \dots, o_i, s_i^* = k)\)
       - The probability that the most likely path is at state $k$ after
         emitting the first $i$ observation.
     - As frequently happens with DP, this table only gets us the
       probabilities.  We'll need to maintain a separate table full of pointers
       if we'd like to get the actual state sequence.  But that's details.
     - Initialize: \(\gamma_{START}(0) = 1\), and for the other $k$, \(\gamma_k(0)=0\).
     - Recursion: \(\gamma_{k}(i) = Pr(o_i | s_i^* = k) \max_p \gamma_p(i-1) Pr(s_i^* = k |
       s_{i-1}^* = p)\)
       - Critical difference here from the forward algorithm is that we want to
